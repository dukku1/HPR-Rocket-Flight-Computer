...
//V2_1_2 adds support for the BMP388
//V2_2_0 adds support for the MPL3115A2, faster rotation computations, and improved packet
//V2_2_1 uses the BMP280 as primary baro sensor and orients accelerometers for Z-axis up
...
//1300Hz of flight events
//1300Hz of integrated speed, altitude, rotation, continuity
...
byte beepPin = 23;//OK
byte buttonGnd = 3;//OK
byte buttonRead = 6;//OK
const byte magSwitchPin = 21;//OK
char pyro1 = 'B';
const byte pyro1Cpin = 35;//OK
const byte pyro1Fpin = 34;//OK
char pyro2 = 'I';
const byte pyro2Cpin = 27;//OK
const byte pyro2Fpin = 38;//OK
char pyro3 = 'A';
const byte pyro3Cpin = 39;//OK
const byte pyro3Fpin = 26;//OK
char pyro4 = 'M';
const byte pyro4Cpin = 17;//OK
const byte pyro4Fpin = 16;//OK
//-----------------------------------------
//radio variables
//-----------------------------------------
int16_t packetnum = 0;
...
void setup(void) {
 ...
 //Read the battery votage from the main parachute continuity pin
  ...
  if(!status_BMP280 && testMode){Serial.println(F("BMP280 not detected!"));}
  if(status_BMP280 && testMode){Serial.println(F("BMP280 OK!"));}
  if(!status_MPL3115A2 && testMode){Serial.println(F(" MPL3115A2 not detected!"));}
  if(status_MPL3115A2 && testMode){Serial.println(F("MPL3115A2 OK!"));baroSensor = 2;}
 ...
 if(testMode){Serial.println(F("Reading User Settings from SD Card"));}
  ...
  if(calibrationMode){
    ...
    Serial.print(F("highGzBiasZ: "));Serial.println(highGzBias);}//end calibration mode
 ...
 while (SD.exists(dataString)) {
  ...
  outputFile.print(F(","));
  outputFile.println(F("aX,aY,aZ,gX,gY,gZ,rotnZ,rotnY,rotnX,offVert,vel,alt,events,cont,fire,pin,highGx,highGy,highGz,highGfilter,Alt,press,temp,magX,magY,magZ,lat,lon,speed,gps_alt,gps_angle,satellites,packet"));
  outputFile.sync();
  ...
  for (byte i = 1; i < 101; i++) { 
    ...
    highGsumX0 += highGx - highGxBias;
    highGsumY0 += highGy - highGyBias;
    highGsumZ0 += highGz - highGzBias;
  ...
  gyroBiasY /= 100;
  gyroBiasZ /= 100;
  highGx0 = (int)(highGsumX0 / 100);
  highGy0 = (int)(highGsumY0 / 100);
  highGz0 = (int)(highGsumZ0 / 100);
  ...
  if (accelY0 >= 0) {yawY0 = asin(min(1, (float)accelY0 / (float)g)) * degRad;}
  else {yawY0 = asin(max(-1, (float)accelY0 / (float)g)) * degRad;}

  if (accelX0 >= 0) {pitchX0 = asin(min(1, (float)accelX0 / (float)g)) * degRad;}
  else {pitchX0 = asin(max(-1, (float)accelX0 / (float)g)) * degRad;}
...
void loop(void){
  ...
  if (!liftoff && accelZ > gTrigger && !touchdown && !timeOut) {
    //timeGyroClock = timeClockPrev; //initializes timeGyro to an appropriate value
    if(testMode){Serial.println(F("Simulated Liftoff Detected!"));}
    lastTX = 0UL;
  ...
  if (liftoff) {
    ...
    if(abs(accelZ) < 31500){accelNow = (float)(accelZ - g) * 0.0071783945;} //0.007178395 = gain * G =  0.000753 *9.80655
    ...
    if (!timeOut && !pyroFire && timeCurrent > rcd_time) {
      ...
      if(!testMode){restoreGPSdefaults();}}
    ...
    if (checkFalseTrigger) {
      ...
      if (accelZ < gTrigger && accelVel < 15.5F) {
        if(testMode){Serial.println(F("False Trigger Reset"));}
    ...
    if (!boosterBurnout && liftoff && accelZ <= 0) {
      boosterBurnout = true;
      radioEvent = 2;
    ...
    if (boosterBurnoutCheck){
      ...
      else if (boosterBurnout && !testMode && accelZ > 0){boosterBurnout = false; boosterBurnoutCheck = false; radioEvent = 1;}}
    ...
    if(twoStage && !apogee && !sustainerBurnout && sustainerIgnition && accelZ < 0 && timeCurrent - timeLastEvent > 100000UL){radioEvent = 14; sustainerBurnout = true; timeLastEvent = timeCurrent;}
    ...
    if(apogeeFire && !mainDeploy && accelZ > 4*g && timeCurrent - timeLastEvent <= 2000000UL){apogeeSeparation = true; radioEvent = 6;}
    ...
    writeIntData(highGy);
    writeIntData(highGz);
    writeFloatData(filterAccel,1);
    //BMP280 Altitude data
    if (newBMP) {
    ...
    if (timeOut || touchdown) {
      ...
      outputFile.println(F("Max Baro Alt,Max GPS Alt,Max Speed,Max Gs,baseAlt,initial Y ang,initial Z ang,accelX0,accelY0,accelZ0,highGx0,highGy0,highGz0,magX0,magY0,magZ0,gyroBiasX,gyroBiasY,gyroBiasZ,accelBiasX,accelBiasY,accelBiasZ,highGxBias,highGyBias,highGzBias"));
      writeLongData(long(maxAltitude*3.2808));
      writeLongData(long(maxGPSalt*3.2808));
      writeLongData(long(maxVelocity*3.2808));
      ...
      writeIntData(highGy0);
      writeIntData(highGz0);
      writeIntData(magX0);
      writeIntData(magY0);
      writeIntData(magZ0);
      ...
      writeIntData(highGyBias);
      writeIntData(highGzBias);
      ...
      dataString[strPosn]=liftoffLat; strPosn++; writeFloatData(liftoffLatitude,4);
      dataString[strPosn]=liftoffLon; strPosn++; writeFloatData(liftoffLongitude,4);
      //end of sample - carriage return, newline, and null value
      dataString[strPosn] = '\r';strPosn++;
      dataString[strPosn] = '\n';strPosn++;
      dataString[strPosn] = '\0';
      ...
      dataString[strPosn]=touchdownLat; strPosn++; writeFloatData(touchdownLatitude,4);
      dataString[strPosn]=touchdownLon; strPosn++; writeFloatData(touchdownLongitude,4);
      //end of sample - carriage return, newline, and null value
  ...
  //GPS Code
  if(!configGPSdefaults && micros() - timeLastGPS > 3000000UL){
    restoreGPSdefaults();
    configGPSdefaults = true; 
    gpsFix = 0;
    fixCount = 0;
    configGPSflight = false;}
  //5 seconds after touchdown put GPS into PSM
  ...
  if (GPS.location.isUpdated() || GPS.altitude.isUpdated()) {
        ...
        fixCount++;
        gpsFix = 1;
        if(!configGPSflight && fixCount > 20){configGPS();configGPSflight = true; configGPSdefaults = false;}
        gpsWrite = true;
        gpsTransmit = true;
        convertLocation();
